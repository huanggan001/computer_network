...
func (z *ZkManager) SetPathData(nodePath string, config []byte, version int32) (err error) {
	exist, _, _ := z.conn.Exists(nodePath)
	if !exist {
...
func (z *ZkManager) RegistServerPath(nodePath, host string) (err error) {
	exist, _, err := z.conn.Exists(nodePath)
	if err != nil {
	...
	if !exist {
		...
		fmt.Println("创建节点成功->", nodePath)
	}
	//临时节点
	subNodePath := nodePath + "/" + host
	exist, _, err = z.conn.Exists(subNodePath)
	if err != nil {
	...
	if !exist {
		...
		fmt.Println("创建节点成功->", subNodePath)
	}
	return
}
